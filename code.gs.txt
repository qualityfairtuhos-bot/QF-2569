...
const PLAIN_TEXT_FIELDS = Object.freeze(['CID','Passport','Phone','LicenseNo','AccountNumber','PromptPay','ReceiptNo','RegID','WorkCode','Postcode']);
...
function seedInitialData() {
  return runSafely_('seedInitialData', function() {
    ...
    if (!findOne_('Conferences',{ConferenceID:cid})) {
      appendRecord_('Conferences',{
        ConferenceID:cid, ConferenceCode:'TUH-QF-2569',
        ...
        ShortName:'TUH Quality 2569', Year:'2569', Organizer:'โรงพยาบาลธรรมศาสตร์เฉลิมพระเกียรติ',
        DescriptionTH:'ระบบลงทะเบียน ส่งผลงาน ประเมินผลงาน และบริหารจัดการงานประชุม',
        DescriptionEN:'Registration, abstract, review and conference management system',
        PrimaryColor:APP.THEME.DEEP_BLUE, SecondaryColor:APP.THEME.TEAL, GreenColor:APP.THEME.GREEN, GoldColor:APP.THEME.GOLD,
        LogoUrl:APP.DEFAULT_LOGO_URL,
        PublicStatus:'DRAFT', Status:'ACTIVE', CreatedAt:now, UpdatedAt:now
      });
    }
...
function defaultSettings_(){ return {
  ...
  EVENT_DATES_JSON:{value:'[]',type:'JSON',group:'SCHEDULE',th:'วันจัดงาน',en:'Event dates'},
...
function seedScoringCriteria_(cid){
  const round=findOne_('ReviewRounds',{ConferenceID:cid,RoundNo:1}); if(!round)return;
  const rows=[
    ['ความสำคัญของปัญหาและวัตถุประสงค์','Problem significance and objectives',20],
    ['วิธีดำเนินงานและความเหมาะสมของกระบวนการ','Methods and process',20],
    ['ผลลัพธ์และหลักฐานเชิงประจักษ์','Results and evidence',25],
    ['การนำไปใช้ประโยชน์และความยั่งยืน','Implementation and sustainability',20],
    ['ความชัดเจนของการนำเสนอ','Clarity of presentation',15]
  ];
  rows.forEach(function(r,i){if(!findOne_('ScoringCriteria',{ConferenceID:cid,ReviewRoundID:round.ReviewRoundID,ItemNo:i+1}))appendRecord_('ScoringCriteria',{CriteriaID:nextId_('CRIT'),ConferenceID:cid,ReviewRoundID:round.ReviewRoundID,ItemNo:i+1,CriteriaNameTH:r[0],CriteriaNameEN:r[1],MaxScore:r[2],WeightPercent:100,RequiredComment:false,Active:true,SortOrder:i+1});});
}
...
function setupDriveFolders(){
  return runSafely_('setupDriveFolders',function(){
    const props=PropertiesService.getScriptProperties();
    let rootId=props.getProperty(APP.PROPERTY_KEYS.ROOT_FOLDER_ID)||APP.ROOT_FOLDER_ID;
    if(!rootId) return {success:false, message:'No ROOT_FOLDER_ID'};
    try {
      const root = DriveApp.getFolderById(rootId);
      FOLDERS.forEach(function(f){
        const it = root.getFoldersByName(f);
        if(!it.hasNext()) root.createFolder(f);
      });
      return {success:true};
...
function getRecords_(name){
  ...
  return globalThis.__TUH_RECORDS[name]=values.map(function(r,idx){ const o={__row:idx+2}; hm.headers.forEach(function(h,i){o[h]=r[i];}); return o; });
}
...
function appendRecord_(name,obj){
  ...
  const next=sh.getLastRow()+1; sh.getRange(next,1,1,row.length).setValues([row]); applyPlainTextToRow_(sh,hm,next,obj); clearRequestCache_(); return next;
}
function updateRecord_(name,rowNumber,patch){
  const sh=getSheet_(name), hm=headerMap_(name); Object.keys(patch).forEach(function(k){ if(hm.map[k]!==undefined){ const cell=sh.getRange(rowNumber,hm.map[k]+1); if(PLAIN_TEXT_FIELDS.indexOf(k)>=0)cell.setNumberFormat('@'); cell.setValue(patch[k]); }}); clearRequestCache_(); return true;
}
...
function invalidateCache_(conferenceId){ CacheService.getScriptCache().removeAll(['PUBLIC_'+conferenceId,'DASH_'+conferenceId]); }
...
function loginUser(username,password,conferenceId,clientInfo){
  return runSafely_('loginUser',function(){
    ...
    return {token:token,conferenceId:cid,role:resolvedRole,user:{UserID:user.UserID,FullName:user.FullName,Email:user.Email,Role:resolvedRole}};
  });
}
...
function getCanonicalWebAppUrl_(){
  ...
  let deployed='';
  try{deployed=normalizePublicWebAppUrl_(ScriptApp.getService().getUrl()||'');}catch(ignore){}
  return deployed;
}
function setPublicWebAppUrl(url){
  return runSafely_('setPublicWebAppUrl',function(){
    ...
    if(!normalized)throw new Error('กรุณาใส่ URL Web App ที่ลงท้ายด้วย /exec เท่านั้น');
...
function buildWebAppRouteUrl_(page,conferenceId,baseUrl,extra){
  const base=normalizePublicWebAppUrl_(baseUrl||getCanonicalWebAppUrl_());
  if(!base)return '';
  const params=[];
  const p=clean_(page||'public').toLowerCase();
  if(p&&p!=='public')params.push('page='+encodeURIComponent(p));
  params.push('conferenceId='+encodeURIComponent(conferenceId||APP.DEFAULT_CONFERENCE_ID));
...
function getExternalAccessLinks(conferenceId){
  return runSafely_('getExternalAccessLinks',function(){
    ...
    if(!base)throw new Error('ยังไม่ได้กำหนด PUBLIC_WEB_APP_URL กรุณารัน setPublicWebAppUrl ด้วย URL /exec');
...
function testExternalBrowserAccessV21(){
  return runSafely_('testExternalBrowserAccessV21',function(){
    ...
    return {success:!!base,version:APP.VERSION,canonicalUrl:base,links:links,requirements:['Deploy เป็น Web app','Execute as: Me','Who has access: Anyone','ใช้ URL /exec โดยตรง','Google Sites ใช้ปุ่มลิงก์หรือ Embed URL /exec เท่านั้น']};
  });
}
function upgradeTuhExternalBrowserAccessV21(publicExecUrl){
  return runSafely_('upgradeTuhExternalBrowserAccessV21',function(){
    ...
    if(!normalized)throw new Error('หลัง Deploy ให้คัดลอก URL ที่ลงท้าย /exec แล้วรัน upgradeTuhExternalBrowserAccessV21(URL)');
    PropertiesService.getScriptProperties().setProperty(APP.PROPERTY_KEYS.PUBLIC_WEB_APP_URL,normalized);
...
function doGet(e){
  ...
  t.directUrl=buildWebAppRouteUrl_(page,cid,base);
  return t.evaluate()
...
function adminGetReviewer(token,conferenceId,reviewerId){
  return runSafely_('adminGetReviewer',function(){
    ...
    const r = findOne_('Reviewers',{ConferenceID:conferenceId,ReviewerID:reviewerId});
...
function adminUpdateReviewer(token,conferenceId,reviewerId,data){
  return runSafely_('adminUpdateReviewer',function(){
    ...
    updateRecord_('Reviewers',r.__row,Object.assign({},data,{UpdatedAt:new Date()}));
    return {success:true};
  });
}
function adminResendReviewerCreds(token,conferenceId,reviewerId){
  return runSafely_('adminResendReviewerCreds',function(){
    ...
    const r = findOne_('Reviewers',{ConferenceID:conferenceId,ReviewerID:reviewerId});
    if(!r)throw new Error('Not found');
    const u = findOne_('Users',{Email:r.Email});
...
function ensureRegFolder_(conferenceId, regId, subFolderType) {
  const s = findOne_('Settings', {ConferenceID: conferenceId, SettingKey: 'DRIVE_ROOT_FOLDER_ID'});
  let rootFolder;
  if (s && s.SettingValue) {
...
function adminSearchDriveFiles(token, conferenceId, query) {
  return runSafely_('adminSearchDriveFiles', function(){
    ...
    let files = [];
    if (!query) return [];
    const queryEsc = String(query).replace(/""/g, '\\"');
    
    const folders = DriveApp.searchFolders('title = "' + queryEsc + '" and trashed = false');
    if (folders.hasNext()) {
      const fIter = folders.next().getFiles();
      let limit = 20;
...
function adminGetRegistrationSignSheet(token, conferenceId, options) {
  return runSafely_('adminGetRegistrationSignSheet', function() {
    ...
    rows.sort(function(a, b) { return String(a.FullName || '').localeCompare(String(b.FullName || ''), 'th'); });
