...
function rateLimited(request:NextRequest){
  ...
  bucket.count+=1;return bucket.count>90;
}
...
async function callGas(payload:RpcRequest&{secret:string},attempts:number){
  ...
  for(let attempt=0;attempt<attempts;attempt+=1){
    ...
    try{
      const response=await fetch(url,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload),redirect:"follow",cache:"no-store",signal:controller.signal});
